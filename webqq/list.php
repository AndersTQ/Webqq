...
var myXmlHttpRequest=getXmlHttpObject();
	...
	if(myXmlHttpRequest){
		...
		//const obj_lis = document.getElementById("list").getElementsByTagName("li");
		
		//见鬼了，放在函数外面就不行，里面就可以
		//const map1=obj_lis.map((element) => {  })
        //var data=new Array();
		...
		//见鬼le，data 的数据逐个增加一个逗号

		//}
		//window.alert(data);
        data="getter=<?php echo $loginuser; ?>";
		...
		myXmlHttpRequest.onreadystatechange=function (){
			...
			//var cons=messageXML.getElementsByTagName("webmid");
			//window.alert(cons.length);
			var stimes=messageXML.getElementsByTagName("sendtime");
			var sender=messageXML.getElementsByTagName("sender");
			...
			//var cons=myXmlHttpRequest.responseText;
						...
						if(cons.length!=0){
								for(var i=0;i<cons.length;i++){
											var str=sender[i].childNodes[0].nodeValue+"对<?php echo $loginuser; ?>说:"+cons[i].childNodes[0].nodeValue+"      "+stimes[i].childNodes[0].nodeValue;
											$(sender[i].childNodes[0].nodeValue).value+=str+"\r\n";
											//$('sender[i].childNodes[0].nodeValue').value+=str+"\r\n";
											//window.alert(str);
...
var addName=decodeURI(loc.substr(n2+1, n1-n2));//从=号后面的内容

var List1 = document.querySelector("#sentList");
